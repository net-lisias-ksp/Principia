...
Checking `Inertially fixed` is useful for spin stabilized burns where your vessel keeps pointing in a fixed direction during the burn. This needs to be checked even if your burn is modeled as `Instant Impulse`, because the latter isn't really instant but just very short.
